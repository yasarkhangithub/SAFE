...
public class SAFESourceSelection {
	 ...
	 */
	 ...
	 /**
	 * Perform source selection using the predicate, source grouping specified at settings/specifications.n3 and ASK queries.
	 ...
	 */
		public void doSAFESourceSelection(List<StatementPattern> stmts) throws RepositoryException, MalformedQueryException, QueryEvaluationException, RDFParseException {
		 ...
		 if (!p.equals("null"))  //--- if predicate is bound
			 {
				...
				//Set<String> lstSources=  Config.uniqueProperties.keySet();  //retrieve list of sources that contains at least one unique property
							...
							(((isStarJoin(stmt, srcUniqueProperties)==true || isPathJoin(stmt, srcUniqueProperties)==true) ||(isStarJoin(stmt,Sets.symmetricDifference(Config.allUniqueProperties,srcUniqueProperties))==false && isPathJoin(stmt,Sets.symmetricDifference(Config.allUniqueProperties,srcUniqueProperties))==false))))
					{
						...
						Boolean isBelongToUniqueSrcGrph =false;
							...
							if(isBelongToUniqueSrcGrph ==false)  //then select all graphs for the current source
							{
								...
								//System.out.println(stmt.getPredicateVar()+ " graphs: "+Config.graphs.get(src));
							}
					}
...
//			System.out.println(graphs);
	 ...
	 * @param stmt statement
	 * @param graphs List of graphs to be bind
	 */
	public void bindGraphsToStatement(StatementPattern stmt,
	...
	public void createPathJoinGroups(List<StatementPattern> stmts) {
		...
		for (StatementPattern stmt : stmts) 
		{
			if (stmt.getObjectVar().getValue()==null && stmt.getPredicateVar().getValue()!=null)  //--- we are only interested if object is unbound and predicate is bound
			{
...
//	}
	...
	private boolean isStarJoin(StatementPattern stmt, Set<String> lst) {
		...
		for(int i=0;i<lstSJGrp.size();i++)
		{
			if (lst.contains(lstSJGrp.get(i))&& lstSJGrp.contains(pred)&& stmt.getParentNode().toString().contains(lstSJGrp.get(i)))
			{
...
//			System.out.println("Stmt: " + stmt.toString() + " ===== Graph Name: " + graphName);
